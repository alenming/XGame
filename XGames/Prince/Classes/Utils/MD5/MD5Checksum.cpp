 ...
 *****************************************************************************************/
std::string MD5Checksum::GetMD5(const std::string& strFilePath)
{
    FILE *file;
    file = fopen(strFilePath.c_str(), "rb");
    if(!file)
        return "";
    MD5Checksum MD5Checksum;   //checksum object
    ...
    //finalize the checksum and return it
...
/*****************************************************************************************
 ...
 DETAILS:  private
 DESCRIPTION: Rotates the bits in a 32 bit uint32_t left by a specified amount
 RETURNS:  The rotated uint32_t
 ARGUMENTS:  uint32_t x : the value to be rotated
 int n   : the number of bits to rotate by
 *****************************************************************************************/
uint32_t MD5Checksum::RotateLeft(uint32_t x, int n)
{
	//check that uint32_t is 4 bytes long - true in Visual C++ 6 and 32 bit Windows
	ASSERT( sizeof(x) == 4 );
...
/*****************************************************************************************
 ...
 RETURNS:  none
 ARGUMENTS:  uint32_t &A, B, C, D : Current (partial) checksum
 uint32_t X           : Input data
 uint32_t S     : MD5_SXX Transformation constant
 uint32_t T     : MD5_TXX Transformation constant
 NOTES:   None
 *****************************************************************************************/
void MD5Checksum::FF( uint32_t& A, uint32_t B, uint32_t C, uint32_t D, uint32_t X, uint32_t S, uint32_t T)
{
	uint32_t F = (B & C) | (~B & D);
	A += F + X + T;
	A = RotateLeft(A, S);
...
/*****************************************************************************************
 ...
 NOTES:   None
 *****************************************************************************************/
void MD5Checksum::GG( uint32_t& A, uint32_t B, uint32_t C, uint32_t D, uint32_t X, uint32_t S, uint32_t T)
{
	uint32_t G = (B & D) | (C & ~D);
	A += G + X + T;
	A = RotateLeft(A, S);
...
/*****************************************************************************************
 ...
 NOTES:   None
 *****************************************************************************************/
void MD5Checksum::HH( uint32_t& A, uint32_t B, uint32_t C, uint32_t D, uint32_t X, uint32_t S, uint32_t T)
{
	uint32_t H = (B ^ C ^ D);
	A += H + X + T;
	A = RotateLeft(A, S);
...
/*****************************************************************************************
 ...
 NOTES:   None
 *****************************************************************************************/
void MD5Checksum::II( uint32_t& A, uint32_t B, uint32_t C, uint32_t D, uint32_t X, uint32_t S, uint32_t T)
{
	uint32_t I = (C ^ (B | ~D));
	A += I + X + T;
	A = RotateLeft(A, S);
 ...
 *****************************************************************************************/
void MD5Checksum::ByteTouint32_t(uint32_t* Output, BYTE* Input, UINT nLength)
{
	//entry invariants
    ASSERT( nLength % 4 == 0 );
...
void MD5Checksum::Transform(BYTE Block[64])
{
	...
	//copy BYTES from input 'Block' to an array of uint32_tS 'X'
	uint32_t X[16];
...
/*****************************************************************************************
 FUNCTION:  MD5Checksum::uint32_tToByte
 DETAILS:  private
 DESCRIPTION: Transfers the data in an 32 bit array to a 8 bit array
 RETURNS:  void
 ARGUMENTS:  BYTE* Output  : the 8 bit destination array
 uint32_t* Input  : the 32 bit source array
 UINT nLength  : the number of 8 bit data items in the source array
 NOTES:   One uint32_t from the input array is transferred into four BYTES
 in the output array. The first (0-7) bits of the first uint32_t are
 ...
 so that there is a perfect fit of 8 bit BYTES into the 32 bit uint32_ts.
 *****************************************************************************************/
void MD5Checksum::uint32_tToByte(BYTE* Output, uint32_t* Input, UINT nLength )
{
	//entry invariants
	ASSERT( nLength % 4 == 0 );
 ...
 *****************************************************************************************/
void MD5Checksum::Update( BYTE* Input, uint32_t nInputLen )
{
...
std::string MD5Checksum::GetMD5OfString(std::string strString)
{
    MD5Checksum MD5Checksum;   //checksum object
