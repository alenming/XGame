...
JNIEXPORT jint JNICALL Java_com_zcjoy_prince_Update_patcher(JNIEnv* env,
		jobject othis, jstring argv1, jstring argv2, jstring argv3) {
	char ** argv;
	int loopVar,result;

	__android_log_print(ANDROID_LOG_INFO, "bspatch.c", "start patcher...");
	...
	result=bspatch(argv);
	__android_log_print(ANDROID_LOG_INFO, "bspatch.c", "patch result %d", result);
