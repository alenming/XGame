...
void TEACrypt::setKey(uint32_t key[])
{
	memcpy(mKey, key, 16);
}
...
void TEACrypt::decrypt(const char* src, size_t size)
{
 ...
 uint32_t TEACrypt::charToUint32(char c1, char c2, char c3, char c4) 
 {
...
void TEACrypt::uint32ToChar(uint32_t n1, uint32_t n2, char src[])
{
...
void TEACrypt::encrypt(uint32_t v[], uint32_t k[])
{
...
void TEACrypt::decrypt(uint32_t v[], uint32_t k[])
{
